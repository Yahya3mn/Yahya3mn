"""
Render "YAHYA" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as
an SVG that animates on GitHub (SMIL only -- GitHub runs SVG animations in
<img>, but never JS).
...
# blank rows above and below the art, so the panel roughly matches a 370-wide
# portrait's height when the two sit side by side.
ROW_MARGIN = int(os.environ.get("WORDMARK_ROW_MARGIN", 4))
CELL_W = 9.0
CELL_H = 15.5
# DejaVu Sans Bold: bundled in scripts/fonts/ so the render is identical
# locally and in CI, independent of whatever fonts happen to be installed on
# the runner. Even stroke weight keeps shading consistent across a letter.
FONT_PATH = os.environ.get("WORDMARK_FONT", os.path.join(HERE, "fonts", "DejaVuSans-Bold.ttf"))
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 0))
TEXT = os.environ.get("WORDMARK_TEXT", "YAHYA")

MASK_H = 300           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.10        # extra letter-spacing, in em. counter gaps must survive the
...
TILT_DEG = float(os.environ.get("WORDMARK_TILT", 4.0))
                       # fixed X tilt so the top face stays visible.
...
RAMP = " .`:-=+*csS#%@"
# keyed close to the view axis, lifted a little: letter faces stay solid and
# dense, the extruded top/side walls fall away to a dimmer char.
...
def build_shell():
    ...
    pts, nrm = [], []
    # the front cap sits a hair proud of z=0, where the side walls begin.
...
def project(P, N, yaw):
    ...
    # depth fog: the far end of a wide wordmark dims, which sells the
...
# ---------------------------------------------------------------------- svg
def emit(frames, mode, out, dur, reveal, title_text):
    art_w = COLS * CELL_W
    art_h = ROWS * CELL_H
...
def main():
    ...
    ap.add_argument("--title", default="yahya@github: ~$ ./wordmark.sh --3d")
    ...
    emit(frames, a.mode, out, dur, a.reveal, a.title)
